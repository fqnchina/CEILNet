...
--GPU 1
model1 = torch.load('/mnt/codes/reflection/models/model_denoise_ecnn_25.net')
model2 = torch.load('/mnt/codes/reflection/models/model_denoise_icnn_25.net')
...
local file = '/mnt/codes/reflection/models/train_denoise_combine.lua'
  ...
  for t = 1,trainsetSize,batch_size do
    ...
    local feval = function(x_new)
      ...
      for i = 1,size do
        ...
        local noise = torch.CudaTensor(1,3,height,width)
        noise:normal(0,0.1)
        label[1] = tempInput
        input[1] = torch.add(label,noise)
        input = input * 255
        label = label * 255
        inputs[{{},{1,3},{},{}}] = input
